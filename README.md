...
- **Детекция сцен** — автоматическое разбиение длинных видео на сцены (PySceneDetect).
- **Многослойный скоринг** — 7 слоёв анализа: аудио, движение, эмоции, объекты, смена планов, биты, яркость.
- **Настраиваемое количество клипов** — от 3 до 20 клипов из одного видео.
- **Композиция из папки** — объединение фрагментов из 30+ видео по темам (CLIP-кластеризация).
- **Самообучение** — система запоминает стиль референсов и применяет веса скоринга.
- **Прогресс в реальном времени** — процент и оставшееся время в интерфейсе.
- **Динамические таймауты** — FFmpeg и yt-dlp адаптируются к длительности видео.
- **Быстрый режим** — анализ в 640x360 для ускорения длинных видео.
- **Бэкенд** — FastAPI + SQLite + Alembic.
- **Фронтенд** — Next.js + Tailwind CSS (тёмная/светлая тема).
