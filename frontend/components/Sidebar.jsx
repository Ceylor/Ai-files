...
export default function Sidebar({ theme, onToggleTheme }) {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 flex w-60 flex-col border-r border-slate-200/60 bg-white/60 backdrop-blur-2xl dark:border-white/5 dark:bg-night-900/70">
      {/* Логотип */}
      <div className="flex h-16 items-center gap-2 border-b border-slate-200/60 px-5 dark:border-white/5">
                ...
                return (
                  <Link
                    ...
                    className={`group relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                      active
                        ? "nav-active text-white dark:text-white"
                        : "text-slate-600 hover:text-slate-900 hover:translate-x-1 dark:text-slate-400 dark:hover:text-white"
                    }`}
                  >
                    <span
          ...
          <span
            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
              theme === "dark"
                ...
                : "bg-gradient-to-r from-neon-cyan to-neon-violet/60 shadow-neon-cyan"
            }`}
          >
            <span
