...
export default function BatchDetails({ batchId }) {
  ...
  if (loading) return <div className="py-10 text-center text-slate-500">Загрузка деталей...</div>;

  const percent = status && status.total_videos
    ...
    <Card title={`Задача #${batchId}`} subtitle={status?.folder_path || ""}>
      ...
      <div className="mt-5">
        ...
        {!results?.videos?.length ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">Видео не найдены</p>
        ) : (
            ...
            {results.videos.map((v) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400">id: {v.id}</div>
                </div>
